...
// ⚠️ ВСТАВЬ СВОЙ ТОКЕН СЮДА ⚠️
const TOKEN = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJ0ZXN0QGV4YW1wbGUuY29tIiwiZXhwIjoxNzcxMTIzOTc1fQ.dS_I56yRjTRZ0_ktdi4d_-_HPoNLgOgcATsCw_40ClU';

// Наши 15 товаров (цены в долларах)
const items = [
  {
...
async function addItems() {
  ...
  try {
    // Сначала получим вишлисты пользователя
    const wishlistsResponse = await axiosInstance.get('/wishlists');
    const wishlists = wishlistsResponse.data;

    if (wishlists.length === 0) {
      console.log('❌ У пользователя нет вишлистов. Сначала создайте вишлист через интерфейс.');
      return;
    }

    const wishlistId = wishlists[0].id; // Берём первый вишлист
    console.log(`📋 Добавляем товары в вишлист ID: ${wishlistId}`);
    ...
    console.log('🎉 Готово! Товары добавлены.');
  } catch (error) {
    console.error('❌ Ошибка при получении вишлистов:', error);
  }
}
